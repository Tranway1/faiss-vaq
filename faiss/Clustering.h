...
/** generates an index suitable for clustering when called */
struct HierarchicalBinIndexFactory {
    /// ownership transferred to caller
    virtual Index* operator()(int dim);

    virtual ~HierarchicalBinIndexFactory() {}
};

/** K-means clustering with hierarchical binary dimension
 *
 * The clustering first happens in hierarchical binary fashion
 */
struct HierarchicalBinClustering: ClusteringParameters {
    typedef Index::idx_t idx_t;
    size_t d;              ///< dimension of the vectors
    size_t k;              ///< nb of centroids
    ...
    HierarchicalBinClustering (int d, int k, const ClusteringParameters &cp);

    /** run k-means training
     *
     * @param x          training vectors, size n * d
     */
    virtual void train (idx_t n, const float * x, HierarchicalBinIndexFactory& factory);


    virtual ~HierarchicalBinClustering() {}
};
