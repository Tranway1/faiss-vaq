...
def main():
  ...
  for i in range(q_len):
    if (len(groundtruth[i]) != len(answers[i])):
      answers[i] = answers[i][:(len(groundtruth[i]))]
      continue
      print(i)
      print(len(groundtruth[i]), len(answers[i]))
  ...
  if (False):
    ...
    print(computeAvg_recall(groundtruth, answers, k), computeRecallAtR(groundtruth, answers, k), computeMAP(groundtruth, answers, k), sep=',')
