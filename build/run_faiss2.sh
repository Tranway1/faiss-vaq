...
if [ "$2" = "ASTRO" ]; then
  ...
  run_basic 256 $DATASET_SIZE 100 $INDEX\
    ...
    "answer/ASTRO_${DATASET_SIZE_H}_${INDEX}.csv" 100
elif [ "$2" = "SEISMIC" ]; then
  if [ "$3" = "100M" ]; then
  ...
  run_basic 256 $DATASET_SIZE 100 $INDEX\
    ...
    "answer/SEISMIC_${DATASET_SIZE_H}_${INDEX}.csv" 100
elif [ "$2" = "SALD" ]; then
  if [ "$3" = "100M" ]; then
    DATASET_SIZE=100000000; DATASET_SIZE_H=100M
  elif [ "$3" = "1M" ]; then
  ...
  run_basic 128 $DATASET_SIZE 100 $INDEX\
    ...
    "answer/SALD_${DATASET_SIZE_H}_${INDEX}.csv" 100
elif [ "$2" = "DEEP" ]; then
  if [ "$3" = "100M" ]; then
    DATASET_SIZE=100000000; DATASET_SIZE_H=100M
  elif [ "$3" = "1M" ]; then
    DATASET_SIZE=1000000; DATASET_SIZE_H=1M 100
  fi
  run_basic 96 $DATASET_SIZE 100 $INDEX\
    $DEEP1B_DATASET_PATH/deep1b_data.bin $DEEP1B_DATASET_PATH/deep1b_queries.bin\
    "answer/DEEP_${DATASET_SIZE_H}_${INDEX}.csv" 100
elif [ "$2" = "SIFT" ]; then
  if [ "$3" = "100M" ]; then
    DATASET_SIZE=100000000; DATASET_SIZE_H=100M
  elif [ "$3" = "1M" ]; then
